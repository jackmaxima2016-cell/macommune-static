...
// 2. Si payée : envoie la commande complète par email à info@macommune.prim.net
// Zéro secret dans le dépôt : STRIPE_SECRET_KEY est injectée par Cloudflare Pages.

const FORM_MAILTO = 'info@macommune.prim.net';
...
export async function onRequestGet(context) {
  ...
  try {
    ...
    // Email de commande → info@macommune.prim.net
    const fd = new FormData();
    fd.append('_subject', '🛒 PAIEMENT REÇU — Nouvelle commande macommune.prim.net');
    ...
    const mail = await fetch(`https://formsubmit.co/ajax/${FORM_MAILTO}`, {
      method: 'POST',
      headers: { Accept: 'application/json', Origin: 'https://macommune.prim.net', Referer: 'https://macommune.prim.net/contact/merci/' },
      body: fd,
    });
