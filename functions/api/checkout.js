...
const SITE = 'https://macommune.prim.net';
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const items = [
      { name: `Pack ${packName} — macommune.prim.net`, price: packPrice },
      ...(dureePrice > 0 ? [{ name: `Durée de publication : ${duree} — macommune.prim.net`, price: dureePrice }] : []),
      ...chosen.map((label, i) => ({ name: `Option : ${label}`, price: OPTIONS[Object.keys(OPTIONS).filter((k) => OPTIONS[k].label === label)[0]].price })),
    ];
    items.forEach((it, i) => {
      params.set(`line_items[${i}][quantity]`, '1');
      params.set(`line_items[${i}][price_data][currency]`, 'chf');
